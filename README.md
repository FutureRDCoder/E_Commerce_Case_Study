...
└── frontend/                       # React SPA
    ...
    └── src/
        ...
        ├── routes/                 # ProtectedRoute, AdminRoute, TenantAdminRoute, CustomerRoute…
...
- `config/DataInitializer` — seeds 14 brand tenants (Samsung, Sony, IKEA, UNIQLO, LEGO, Canon, Bose, The North Face, Apple, Nike, Adidas, Puma, Reebok, Levi's) with sample products on first startup, only when the database is empty (`dev` profile).
...
Deleting a brand or a product is a **soft delete**: the record is merely deactivated (an `active` flag), never removed from the database.

- A deactivated brand disappears from `/brands` and the storefront, but its **orders and customer favourites remain visible forever** — customers can still see their order history and favourited products.
- A deactivated product disappears from the storefront and cannot be added to the cart or ordered, but **favourites and order history referencing it are preserved**.
- The `ADMIN` **All Orders** view still lists every order ever placed, whether or not the brand still exists.
...
4. **Favourites:** click the heart icon on a product, then view them under **Favourites**. Favourites survive even if the product or its brand is later deleted.
5. **Cart:** add items to the cart (cart badge updates), adjust quantities, and remove items on the cart page.
6. **Order:** place an order from the cart → stock is decremented and the order is created. View your order history under **Orders** (the **Orders** link appears in the top-right navbar for `USER` and `TENANT_ADMIN`).
...
4. **Manage products** (`/tenant/dashboard/products`):
   ...
   - Delete products (soft delete: the product leaves the storefront, but customer favourites and order history for it are preserved).
...
2. Click **Admin** (top-right) → `/admin/dashboard`. The dashboard has three sections:
   - **Brands:** add a new brand (name, slug, description, logo) or delete an existing one. Deleting a brand **soft-deletes** it: it disappears from the storefront, but its historical orders and customers' favourites are preserved forever.
...
4. **Negative check:** an `ADMIN` cannot place orders or use favourites — both are rejected by the backend, and the cart, favourites and orders UI is hidden for admins in the navbar.
...
| `POST/DELETE/GET /{tenantSlug}/favourites` | Authenticated (`ADMIN` excluded)|
